...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $con->prepare("INSERT INTO homework_assignments (title, subject, description, deadline, class, session) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $title, $subject, $description, $deadline, $class_id, $session);
    
    if ($stmt->execute()) {
        $_SESSION['success_message'] = "Homework assignment added successfully!";
        header("Location: addimage.php");
        exit();
    } else {
        $error_message = "Error adding homework assignment: " . $con->error;
    }
}
?>
...
<head>
  <meta charset="utf-8">
  <title>Admin | Add Homework</title>
              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <select name="class_id" class="form-control">
                    ...
                    <option value="<?php echo $class['class']; ?>">Class <?php echo htmlspecialchars($class['class']); ?></option>
                    <?php endwhile; ?>
                  </select>
                </div>
                <div class="form-group">
                  ...
                  <select name="session" class="form-control">
                    ...
                    <option value="<?php echo $session['session']; ?>">Session <?php echo htmlspecialchars($session['session']); ?></option>
                    <?php endwhile; ?>
                  </select>
                </div>
                <div class="form-group">
