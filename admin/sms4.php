<?php
include('config.php');


$course=$_POST['course'];
$msg=$_POST['msg'];

$date=date('d-m-Y');
  

 if($con->error)
	echo $con->error;
	else
	{
	$sql="select * from admission where course='$course'";
	$res=$con->query($sql);
	while($row=$res->fetch_array())
	{
		
	$s_name=$row['student_name'];	
    $phone_number=$row['phone'];
    $student_code=$row['code_no'];
	
	
	 $sql2="insert into fees_reminder(id,student_name,course,student_code,msg,date) values(null,'$s_name','$course','$student_code','$msg','$date')";
	 $con->query($sql2); 
	
 
 
 $mobileNumber = "$phone_number";
$senderId = "AKI";
$message = urlencode("$msg.Ignore if you allready paid .Date: $date. For more info Ak Infotech  9051525611.");
$route = 4;
$postData = array(
    'mobiles' => $mobileNumber,
    'message' => $message,
    'sender' => $senderId,
    'route' => $route
);

$url="http://sms.indcomsoftweb.com/api/v2/sendsms";
$curl = curl_init();
curl_setopt_array($curl, array(
    CURLOPT_URL => "$url",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_CUSTOMREQUEST => "POST",
    CURLOPT_POSTFIELDS => $postData,
    CURLOPT_HTTPHEADER => array(
        "authkey: 13438A57SeUqbWPz5d88b7fb",
        "content-type: multipart/form-data"
    ),
));
$response = curl_exec($curl);
$err = curl_error($curl);
curl_close($curl);
if ($err) {
    echo "cURL Error #:" . $err;
} else {

	
}
 

	}}
	
	
	
 
echo "<script>window.location.href='exam_reminder.php'</script>"; 

?>



                        
                        

     