  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Add Student Fees</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Add Student Fees</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
...
elseif(isset($_SESSION['s_course2']))
{
	$course=$_SESSION['s_course2'];
	$branch=$_SESSION['s_branch2'];
}
...
?>
			...
			<tbody>
				...
				if($con->error)
				  ...
				  $sql="select distinct student_code from fees_table where course='$course' and branch='$branch' ORDER BY id DESC";
				  
				  $res = $con->query($sql);
				  while($rows = $res->fetch_assoc()){
					  $s_code=$rows['student_code'];
					  //$f_course=$rows['course'];
					  $_SESSION['s_course2']=$course;
					  $_SESSION['s_branch2']=$branch;
				?>
				<tr>
				<?php
				$sql2="select * from admission where course='$course' and code_no='$s_code' and branch='$branch'";
				  
				  $res2=$con->query($sql2);
				  $row2=$res2->fetch_assoc();
				  
				  $sql3="select * from fees_table where course='$course' and student_code='$s_code' and branch='$branch' ORDER BY id DESC LIMIT 1";
				  
				  $res3=$con->query($sql3);
				  $row3=$res3->fetch_assoc();
				  //echo $row3['month'];
				?>
		<td><img style="height:70px; width:100px" src="<?php echo $row2['image']; ?>"></td>
		 ...
		 <td><a href="fees_remind_by_month.php?id=<?php echo $row3['student_code']?> && course=<?php echo $row2['course']?> && month=<?php echo $row3['month'];?>"><button class="btn btn-info">
			<i class="fa fa-envelope"></i></button></a></td>
			<td><a href="allfees.php?id=<?php echo $row2['code_no']?> && course=<?php echo $row2['course']?>"><button class="btn btn-warning">
